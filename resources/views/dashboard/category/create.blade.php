<x-dashboard-layout title="Tambah Data Kategori">
    <x-slot name="header">
    ...
    <div class="card p-4">
        <form method="post" action="{{ route('dashboard.category.store') }}" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control {{ $errors->get('name') ? 'border-danger' : '' }}" id="name" name="name" placeholder="Nama Kategori" value="{{ old('name') }}" autocomplete="name" autofocus required />
            ...
            <div class="mb-3">
                ...
                <div id="requirement-fields">
                    @if(old('requirements'))
                        ...
                        @endforeach
                    @else
                        <div class="row mb-3 requirement-item" data-index="0">
                            <div class="col-md-6">
                                <label class="form-label" for="requirements[0][name]">Nama Persyaratan 1 <span style="font-size:14px;color:red">*</span></label>
                                <input type="text" class="form-control" name="requirements[0][name]" id="requirements[0][name]" placeholder="Nama Persyaratan" required />
                            </div>
                            <div class="col-md-6">
                                <label class="form-label" for="requirements[0][file]">Contoh/Format Persyaratan 1</label>
                                <input type="file" class="form-control" name="requirements[0][file]" id="requirements[0][file]" />
                            </div>
                            <div class="col-12 text-end mt-1">
                                <button type="button" class="btn btn-danger btn-sm remove-requirement" style="display: none;">Hapus</button>
                            </div>
                        </div>
                    @endif
                </div>
