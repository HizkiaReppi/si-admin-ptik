    ...
    <div class="card mb-4">
        ...
        <div class="card-body" style="margin-bottom: -20px">
            <div class="d-flex flex-column align-items-start gap-4">
                ...
                @else
                    <img src="/{{ $pimpinan_jurusan->user->photoFile }}"
                        alt="{{ $pimpinan_jurusan->fullname }}" class="d-block rounded" style="width: 250px" id="foto" />
                @endif
            </div>
        </div>
        <div class="card-body">
        ...
        <div class="d-flex mb-4 ms-3" style="margin-top: -15px">
            <a href="{{ route('dashboard.pimpinan-jurusan.edit', $pimpinan_jurusan->id) }}" class="btn btn-primary ms-2">Edit Data</a>
            <a href="{{ route('dashboard.pimpinan-jurusan.destroy', $pimpinan_jurusan->id) }}" class="btn btn-danger ms-2"
                data-confirm-delete="true">Hapus Data</a>
            <a href="{{ route('dashboard.pimpinan-jurusan.index') }}" class="btn btn-outline-secondary ms-2">Kembali</a>
        </div>
    </div>
</x-dashboard-layout>
